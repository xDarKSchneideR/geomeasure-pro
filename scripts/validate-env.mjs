...
// ?? Variables CR�TICAS por entorno
const CRITICAL_VARS = {
  frontend: [
    // ?? Estas NO deben contener secretos reales - solo configuraci�n p�blica
    { 
...
// ?? Funci�n de validaci�n de una variable
function validateVar({ name, pattern, minLen, maxLen, allowed, optional, error }, value, envName) {
  ...
  // Caso 2: Validar patr�n regex
  if (pattern && !pattern.test(value)) {
    return { valid: false, error: `? ${envName}: '${name}' tiene formato inv�lido. ${error}` };
  }

  // Caso 3: Longitud m�nima
  if (minLen && value.length < minLen) {
    return { valid: false, error: `? ${envName}: '${name}' es muy corta (m�n. ${minLen} chars, actual: ${value.length})` };
  }
  ...
  // Caso 5: Valores permitidos espec�ficos
...
// ?? Escanear c�digo frontend en busca de secretos expuestos (heur�stica b�sica)
function scanForExposedSecrets(rootDir) {
  ...
  // Escanear archivos espec�ficos que suelen contener configuraci�n
  const filesToScan = [
...
async function main() {
  ...
  // Determinar qu� entornos validar
  ...
  // ?? Check adicional: detectar secretos expuestos en c�digo frontend
  if (mode === 'all' || mode === 'frontend') {
    ...
    if (exposed.length > 0) {
      ...
      log.success('? No se detectaron secretos expuestos en c�digo frontend');
    }
  }
  ...
  if (hasErrors) {
    ...
    if (stats.warn > 0) {
      log.warn('??  Validaci�n completada con advertencias. Revisa las variables opcionales.');
    } else {
...
// Ejecutar
main().catch(err => {
  log.error(`Fallo en ejecuci�n: ${err.message}`);
  if (err.stack) {
